...
class TestIonQClient:
    ...
    def test_custom_base_url(self):
        assert (
            IonQClient(api_key="key", base_url="https://staging.ionq.co/v0.4")._base_url
    ...
    def test_additional_user_agent(self):
        assert (
            "qiskit-ionq/1.0"
            in IonQClient(api_key="key", additional_user_agent="qiskit-ionq/1.0")
            .get_httpx_client()
            .headers["User-Agent"]
        )
    ...
    def test_async_client_wired(self):
        async_client = IonQClient(api_key="key").get_async_httpx_client()
