...
class TestEndpointPathParamRejection:
    ...
    @pytest.mark.parametrize(
        "call",
        [
            lambda c: get_characterization.sync_detailed("qpu.forte-1", "..", client=c),
    ...
    def test_fixed_segments_survive_hostile_ids(self):
        ...
        kwargs = get_characterization._get_kwargs("qpu.forte-1", "v.1")
        assert kwargs["url"] == "/backends/qpu.forte-1/characterizations/v.1"
