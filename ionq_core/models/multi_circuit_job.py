...
from ..models.multi_circuit_job_type import check_multi_circuit_job_type
from ..models.multi_circuit_job_type import MultiCircuitJobType
from ..types import UNSET, Unset
...
if TYPE_CHECKING:
  ...
  from ..models.multi_circuit_job_results import MultiCircuitJobResults
...
T = TypeVar("T", bound="MultiCircuitJob")



@_attrs_define
class MultiCircuitJob:
    ...
    identifier (typically `ionq.result.probabilities-aggregate.json.v1`).
        ...
        Attributes:
            ...
            results (MultiCircuitJobResults | None): Aggregated result artifacts across all child circuits, keyed by format
                identifier. Typically contains `ionq.result.probabilities-aggregate.json.v1` — the merged probability
                distribution over every child circuit. Each entry is an `ArtifactDescriptor` — pass its `id` to the [artifact
                endpoint](/api-reference/v0.4/jobs/get-job-artifact) to download the payload. See [Results formats](/api-
                reference/v0.4/schemas/results-formats) for the catalog of valid identifiers and their payload schemas.
    ...
    settings: CircuitJobSettings
    stats: CircuitJobStats
    results: MultiCircuitJobResults | None
    child_job_ids: list[str] | None
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        if isinstance(self.results, MultiCircuitJobResults):
    ...
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        ...
        type_ = check_multi_circuit_job_type(d.pop("type"))
        ...
        def _parse_results(data: object) -> MultiCircuitJobResults | None:
            if data is None:
                return data
            try:
                if not isinstance(data, dict):
                    raise TypeError()
                results_type_1 = MultiCircuitJobResults.from_dict(data)
                ...
                pass
            return cast(MultiCircuitJobResults | None, data)
