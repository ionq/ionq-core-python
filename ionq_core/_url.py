...
def quote_path_param(value: object) -> str:
    ...
    Raises ``ValueError`` for ``""``, ``"."``, and ``".."``: ``quote`` never encodes dots, so those
    would survive into the URL verbatim and collapse a fixed segment under RFC 3986 normalization
    (e.g. ``/sessions/../jobs`` -> ``/jobs``, turning a session-scoped request into an account-wide one).
    """
    segment = str(value)
    if segment in ("", ".", ".."):
